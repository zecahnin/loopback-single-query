...
module.exports = class SingleQuery {
     ...
     */
    static count(Model, filter = {}, customQbOption = {},  callback) {
        ...
        delete filter.limit;
        delete filter.skip;
        let sql = nqb.createQueryByFilter(Model, filter, customQbOption);
        sql = 'SELECT count(*) as quantidade FROM ( '+ sql + ') as tmp;';
        let data = await SingleQuery._query(Model, sql, []);
        return _.first(data);
    }), [...arguments], callback);
    }
     ...
     * @returns {null|Promise}
     */
    static find(Model, filter = {}, customQbOption = {},  callback) {
        return resolveAsyncFunction((async () => {
            if (_.isFunction(filter)) {
            callback = filter;
            filter = {};
        ...
        const nqb = new QueryBuilder(Model.app);
        const sql = nqb.createQueryByFilter(Model, filter, customQbOption);
        let data = await SingleQuery._query(Model, sql, []);
    ...
    static findOne(Model, filter = {}, callback) {
        ...
        filter.limit = 1;
        const data = await SingleQuery.find(Model, filter);
        return _.first(data);
    }), [...arguments], callback);
    }
