...
module.exports = class ObjetoAprendizagemNativeQueryBuilder {
  ...
  _includeSubQuery(Model, filter, options, rootAlias) {
    ...
    return _.compact(_.map(filter.include, include => {
      ...
      if (query) return {field: `(${query})`, alias: include.relation};
    }));
  }
  ...
  _getWhere(Model, where, rootAlias) {
    ...
    if (where.and) {
      ...
      if (_.isNull(where[fieldName])) {
      ...
      } else if (where[fieldName] && (_.isString(where[fieldName].gt) || _.isNumber(where[fieldName].gt))) {
        return `_${rootAlias}.${dbFieldName} > ${where[dbFieldName].gt}`;
      } else if (where[fieldName] && (_.isString(where[fieldName].gte) || _.isNumber(where[fieldName].gte))) {
        return `_${rootAlias}.${dbFieldName} >= ${where[dbFieldName].gte}`;
      } else if (where[fieldName] && (_.isString(where[fieldName].lt) || _.isNumber(where[fieldName].lt))) {
        return `_${rootAlias}.${dbFieldName} < ${where[dbFieldName].lt}`;
      } else if (where[fieldName] && (_.isString(where[fieldName].lte) || _.isNumber(where[fieldName].lte))) {
        ...
        return `_${rootAlias}.${dbFieldName} NOT IN(${where[dbFieldName].nin.join(',')})`;
      } else if (where[fieldName] && (_.isString(where[fieldName].neq) || _.isNumber(where[fieldName].neq))) {
