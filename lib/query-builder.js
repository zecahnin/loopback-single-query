...
const debug = require('debug')('single-query:builder');
...
module.exports = class ObjetoAprendizagemNativeQueryBuilder {
  ...
  _parseValue(Model, property, value) {
    ...
    } else if (Model.definition.properties[property].type === Array) {
      return _.map(value, v => this._parseValue(v));
    }
    return `'${value}'`;
  }
  ...
  _getWhere(Model, where, rootAlias) {
    ...
    if (where.and) {
      ...
      if (_.isNull(where[fieldName])) {
        ...
        return `_${rootAlias}.${dbFieldName} IN(${this._parseValue(Model, fieldName, where[dbFieldName].inq).join(',')})`;
      } else if (where[fieldName] && where[fieldName].nin) {
        return `_${rootAlias}.${dbFieldName} NOT IN(${this._parseValue(Model, fieldName, where[dbFieldName].nin).join(',')})`;
