  ...
  _getWhere(Model, where, rootAlias) {
    ...
    if (where.and) {
      ...
      if (_.isNull(where[fieldName])) {
        ...
        return `_${rootAlias}.${dbFieldName} > ${this._parseValue(Model, fieldName, where[fieldName].gt)}`;
      } else if (where[fieldName] && isValid(where[fieldName].gte)) {
        return `_${rootAlias}.${dbFieldName} >= ${this._parseValue(Model, fieldName, where[fieldName].gte)}`;
      } else if (where[fieldName] && isValid(where[fieldName].lt)) {
        return `_${rootAlias}.${dbFieldName} < ${this._parseValue(Model, fieldName, where[fieldName].lt)}`;
      } else if (where[fieldName] && isValid(where[fieldName].lte)) {
        return `_${rootAlias}.${dbFieldName} <= ${this._parseValue(Model, fieldName, where[fieldName].lte)}`;
        ...
        return `_${rootAlias}.${dbFieldName} IN(${_.map(where[fieldName].inq, v => this._parseValue(Model, fieldName, v)).join(',')})`;
      } else if (where[fieldName] && where[fieldName].nin) {
        return `_${rootAlias}.${dbFieldName} NOT IN(${_.map(where[fieldName].nin, v => this._parseValue(Model, fieldName, v)).join(',')})`;
      } else if (where[fieldName] && isValid(where[fieldName].neq)) {
        return `_${rootAlias}.${dbFieldName} != ${this._parseValue(Model, fieldName, where[fieldName].neq)}`;
